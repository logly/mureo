...
class TestReadNameExemption:
    ...
    def test_the_exemption_uses_the_shared_read_vocabulary(self) -> None:
        """One prefix list, read through two matchers — see
        mureo.core.tool_names. This surface uses the STRICT matcher; the
        rollback planner uses the looser sibling, which also accepts a verb at
        the END of a name. Both must still agree on a verb-FIRST name like
        this one, which is the part the single-sourced list guarantees."""
