...
class TestListTools:
    ...
    async def test_list_tools_returns_all_tools(self) -> None:
        ...
        + Rollback 2 + Batch 3 + Change Import 1 + Analysis 5 + Mureo Context 12
        + Analytics Registry 2 + Learning 2 + Learning pre-flight 1
        + Creative Studio 5 = 225).

        Mureo Context is 12 since #638 added
        mureo_state_platform_not_collected_set, #661 added its
        document-level counterpart mureo_state_workspace_not_collected_set,
        and #690 added mureo_state_platform_daily_set (the day-grain history
        the window rollups cannot hold).
