...
**What makes the trail hold up (#758 phase 6).** Three properties, one per way it could quietly stop being a record. It is **bounded**: `JOURNAL.jsonl` and each `history/reports/<kind>.jsonl` are rotated away at 32 MiB (`MUREO_JOURNAL_MAX_BYTES`) under a timestamped name, so a long-lived workspace costs a bounded read — and nothing is ever deleted, the bound is on one file rather than on the history. It is **tamper-evident**: every journal record carries the hash of the previous physical line and of itself, the chain continues across a rotation, and `mureo journal --verify` reports the first break with the file and line. The one thing it cannot show is a truncated tail, which is stated in the docs rather than glossed over. And a **whole-document STATE.json write that shortens `action_log` leaves a copy**: `write_state_file` backs the previous document up to `STATE.json.bak.<unix_ns>` and warns, whoever the writer is — a restore, an agency sync or a repair. The write still proceeds (a document that cannot be written is a workspace nobody can repair), but it can no longer happen unnoticed, and a backup that fails stops the write.
...
│   ├── tools_analysis.py                  # analysis_anomalies_check / analysis_delivery_collapse_* (#546) / analysis_tracking_consistency_check
