...
| `MonthlyBudgetSupport` dataclass shape (`platform`, `evidence`), `register_monthly_budget_support` and `MonthlyBudgetSupportWarning` — how a provider declares that its campaigns carry a monthly budget (#656), read together with the optional `CampaignSnapshot.monthly_budget` field. `Evidence` is required; registration is first-wins; core declares no platform. mureo stores no total derived from it. | `mureo.context.platform_monthly_budget` | Stable (additive) |
