...
- **`mureo configure` (browser)** — starts a local UI on `http://127.0.0.1:<random-port>/` and opens your browser. It does the same credential entry (HTML forms + standard OAuth redirects, each field deep-linked to the right console) **and** the rest of Claude setup: pick the host, run basic setup, add official MCP providers, set the active workspace's Guardrails account currency (Advanced → Guardrails card), scaffold Demo/BYOD. Recommended when an AI agent (Claude Code, etc.) pointed you here, or you just prefer a GUI. (The old `mureo auth setup --web` was removed and folded into `mureo configure`.)
