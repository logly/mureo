...
|---|---|---|
| Claude Code | `/daily-check`, `/budget-rebalance`, … | All 15 operational skills + 6 foundation skills available locally |
...
2. For each operational skill, upload the SKILL.md file from `mureo/_data/skills/<name>/SKILL.md` (15 files: `daily-check`, `tracking-health`, `budget-rebalance`, `budget-pacing`, `search-term-cleanup`, `creative-refresh`, `lead-form-create`, `rescue`, `goal-review`, `weekly-report`, `monthly-report`, `competitive-scan`, `onboard`, `sync-state`, `learn`).
