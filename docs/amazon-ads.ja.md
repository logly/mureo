...
- **読み取り自体**は通常のブリッジ呼び出しです。他と同じく認証・トークン
  ...
  ことはありません。
- **探索は 1 本の接続を共有します。** 広告プロダクトを何回探索する場合
  でも、その取得処理のために 1 回だけ開いた**単一**の認証済み MCP
  セッション上で実行されます（読み取りごとにハンドシェイクをやり直す
  ことはありません）。複数回探索する取得処理で時間を食っていたのは
  クエリ本体ではなくハンドシェイクだからです。途中でアクセストークンの
  更新が必要になった場合は、そのセッションを閉じて新しいトークンで開き
  直し、失効に当たった呼び出しだけを再試行します。更新は**1 回の取得
  処理につき最大 1 度**なので、探索の連続がトークン交換の連続に化ける
  ことはありません。通常のツール呼び出しはこれまでどおり、1 呼び出しに
  つき 1 セッションのままです。
