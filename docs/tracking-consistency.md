...
**Failing open is not the same as failing silently.** When the check could not run, the successful create response carries a `tracking_preflight` field saying so in words (`NOT CHECKED: …`), every such miss emits a log record, and three consecutive misses escalate from WARNING to an ERROR stating that the create-time guardrail is effectively off. A permanently broken guardrail and a quiet one look identical otherwise.

This covers **every** way the check can fail to run, not only the ones that raise: a read error, and an account whose ad group mureo could not resolve to a campaign. The second is the one that matters most for this, because an integration returning empty data instead of erroring is exactly the failure that would otherwise stay silent forever. A success resets the streak.
