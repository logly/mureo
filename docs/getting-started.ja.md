...
>
> - **いちばん簡単 — 全部ブラウザから（推奨）:** `pip install mureo` のあと `mureo configure`。ローカルのブラウザ UI（`127.0.0.1` バインド）が開き、**このページの内容をすべてクリック操作で実施**できます — ホスト選択・基本セットアップ・OAuth/認証情報・公式 MCP プロバイダ・Demo/BYOD。ターミナルでのコマンド入力は不要。
> - **手動 — コマンドを自分で実行:** 以下はその設定を**コマンドで1つずつ**たどる方法です。スクリプト/CI・細かい制御が必要な場合向け。
>
