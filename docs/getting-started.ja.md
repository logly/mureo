...
mureo のインストールと起動は、**ターミナル**（PC の「コマンドを打つアプリ」。Claude の中ではありません）から行います。まずターミナルを開きます:

- **macOS:** ⌘ + Space を押して `Terminal` と入力 → Enter。
- **Windows:** スタートメニューを開き `Terminal`（または `PowerShell`）と入力 → Enter。
- **Linux:** お使いのターミナルアプリを開く。

mureo には **Python 3.10 以上** が必要です。ターミナルで次を実行:

```bash
python3 --version
```

`Python 3.10.x` 以上が表示されれば準備OK（`pip` は Python に同梱）。**コマンドが見つからない**、または**バージョンが 3.10 未満**の場合は、**<https://www.python.org/downloads/>** から Python をインストール（`pip` も同梱されます）し、ターミナルを開き直してください。後の手順で `pip: command not found` と出る場合は、`pip` の代わりに `python3 -m pip` を使います。

> **セットアップは2通り — どちらか片方でOK。**
>
> - **いちばん簡単 — 全部ブラウザから（推奨）:** ターミナルでコマンドは2つだけ — `pip install mureo` のあと `mureo configure`。するとローカルのブラウザ UI（`127.0.0.1` バインド）が開き、**このページの残りの設定を全部クリック操作で実施**できます — ホスト選択・基本セットアップ・OAuth/認証情報・公式 MCP プロバイダ・Demo/BYOD。以降コマンド入力は不要。
> - **手動 — コマンドを自分で実行:** 以下はその設定をターミナルで**コマンドで1つずつ**たどる方法です。スクリプト/CI・細かい制御が必要な場合向け。
>
> どちらも到達する状態は同じ。迷ったら `mureo configure` を使ってください。
...
> BYOD import はターミナルから実行します — 下記コマンドをシェルで実行してください。
...
mureo configure                     # ブラウザ UI: OAuth・ホスト設定・プロバイダ
```
...
```bash
pip install mureo
mureo setup codex                   # MCP サーバー + 認証ガードフック + skills を ~/.codex に配置
mureo configure                     # ブラウザ UI: OAuth・認証情報(「OpenAI Codex」ホストを選べばここで全部完結)
```

Codex を再起動 — CLI・IDE 拡張・デスクトップアプリはすべて `~/.codex/config.toml` を共有するので、一度のセットアップで 3 つとも対応します。あとはワークフローを実行(「日次チェックして」、または `/skills` ピッカーから `$daily-check`)。MCP サーバーは `~/.mureo/credentials.json` から認証情報を自動読み込みします。
...
2. 操作系 skill を 1 つずつアップロード: `mureo/_data/skills/<name>/SKILL.md` (20 ファイル: `daily-check` / `tracking-health` / `budget-rebalance` / `budget-pacing` / `search-term-cleanup` / `creative-refresh` / `creative-generate` / `ad-fatigue-check` / `experiment` / `lead-form-create` / `rescue` / `incident-postmortem` / `goal-review` / `weekly-report` / `monthly-report` / `competitive-scan` / `audience-review` / `onboard` / `sync-state` / `learn`)。
