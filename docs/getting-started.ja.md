...
> **いちばん簡単な方法 — 多くの人はここから。** `pip install mureo` のあと `mureo configure` を実行すると、ローカルのブラウザ UI（`127.0.0.1` バインド）が開き、ホスト選択・基本セットアップ・OAuth/認証情報・公式 MCP プロバイダ・Demo/BYOD までまとめて設定できます（ターミナルのフラグ不要）。以下の CLI 手順は手動・スクリプト派／CI・自動化・細かく制御したい人向けの詳細リファレンスで、到達する状態は同じです。
