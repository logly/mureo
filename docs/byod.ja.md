...
> MCP ツールでのみ提供されます。**公式プラットフォーム MCP**（`mureo
> providers add google-ads-official` / `meta-ads-official`、または
> Claude.ai の Meta コネクタ）は **Live API** 経路で、実プラット
> フォームに接続するため、ローカルの BYOD/デモ データは読めません。
> プラットフォーム単位で両者は排他です: BYOD やデモで分析する
> プラットフォームについては、`mureo providers add`（=
> `MUREO_DISABLE_<PLATFORM>=1` を設定）や公式 MCP への切替を**行わず**、
> mureo-native を有効のままにしてください。公式 MCP は Live API 専用に。
> （`mureo providers remove` / ダッシュボードの native↔official トグルで
> mureo-native を再有効化できます。）
...
- 列: キャンペーン名 / 広告セット名 / 広告名 / 結果 / 結果インジケーター /
  消化金額 / インプレッション / リーチ / リンククリック /
  フリークエンシー

**多言語対応**: mureo の Meta アダプタは英語 / 日本語 / 简体中文 /
繁體中文 / 한국어 / Español / Português / Deutsch / Français の **9 言語**
で実エクスポートを検証済みです。列ヘッダの言語は切り替える必要は
ありません。

> **EU ロケールの日付は非対応**: mureo が認識する日付セルは ISO
> (`YYYY-MM-DD`) / スラッシュ ISO (`YYYY/MM/DD`) / 米国式
> (`MM/DD/YYYY`) の 3 形式だけです。列*ヘッダ*は 9 言語すべてで
> 認識されますが、日付の*書式*は別問題です。`DD/MM/YYYY` は 12 日
> 以下だと `MM/DD/YYYY` と区別が付かないため、受け入れると日次
> メトリクスを黙って誤集計（または欠落）してしまいます。アカウント
> 言語が `DD/MM/YYYY` を出力する欧州ロケールの場合は、**レポート →
> アカウント言語**を **English** に切り替えて `MM/DD/YYYY` で
> 書き出してください。

**通貨非依存**: 消化金額の列ヘッダは Meta が `Amount spent (XXX)` /
`消化金額 (XXX)` のように口座の ISO 通貨コード (`JPY` / `USD` /
`EUR` / `GBP` / `KRW` / `INR` 等) を末尾に付けてエクスポートします。
mureo は `(XXX)` 接尾辞を除去してから列名を解決し、セル側の通貨記号
(`¥` / `$` / `€` / `£` / `₩` / `₹` 等) も自動で除去します。コスト値は
口座の通貨単位のまま保存され、mureo は通貨換算を行いません — 同じ
アカウント内の CTR / CPC / CPA はそのまま意味を持ちます。
...
| `mureo byod status` | プラットフォーム別のモード（BYOD / Live API / 未設定）を表示 |
...
といった食い違いがあると mureo は警告を出して Live API モードにフォール
バックします。

---

## BYOD と Live API を併用するパターン

| 状態 | 動作 |
|---|---|
| Google Ads だけ取り込み済 / Meta は未設定 | Google Ads = bundle / Meta = Live API |
| Meta だけ取り込み済 / Google Ads は未設定 | Meta = bundle / Google Ads = Live API |
| 両方取り込み済 | 両方 = bundle |
| 何も取り込まない | 全部 Live API |
| `mureo byod clear` 実行後 | 全部 Live API |

GA4 と Search Console は常に Live API 経由です。Sheet bundle の対象
プラットフォームには含まれません。
...
`set_`, `patch_`, `boost_`, `end_`, `duplicate_`, `export_`

末尾の 4 つ（`boost_` / `end_` / `duplicate_` / `export_`）は、上記の
汎用動詞に一致しない Meta 固有のミューテーション（`boost_post` /
`end_split_test` / `duplicate_lead_form` / `export_leads_to_csv`）を
カバーします。
...
Live API の OAuth トークンは BYOD のリセットを跨いで残ります。削除後は
...
`mureo auth setup`（または `mureo configure`）で
`~/.mureo/credentials.json` を埋めれば Live API で動き始めます。
...
- **パストラバーサル防御**: bundle importer は `~/.mureo/byod/` の外には
  絶対に書き込まない実装になっています
- **CSV インジェクション対策**: すべての BYOD アダプタ（Google Ads /
  Meta）が、ユーザー由来のセル値（キャンペーン / 広告セット / 広告名・
  キーワード・検索語・クリエイティブ文言）を共通サニタイザ
  （`mureo/byod/adapters/_csv_safe.py`）に通します。`=` / `+` / `-` /
  `@` / タブ / 復帰改行 で始まるセルは先頭にシングルクオートを付けて
  無害化し、生成した CSV を Excel / Sheets で開き直したときに数式として
  自動実行されないようにします
...
Meta エクスポートに配信内訳や属性内訳（Step 2b の *配置 /
...
- **GA4 / Search Console** は Live API 認証（`mureo auth setup`）で。
...
Live API の全機能を使いたくなったときは、`mureo auth setup`（または
`mureo configure`）でクレデンシャルを設定し、`mureo byod clear`
で BYOD モードを抜けてください。
...
| ツール | 返すもの |
|---|---|
| `google_ads_search_terms_report` | 検索語ごとに 1 行: campaign_name / ad_group_name / impressions / clicks / cost / conversions / ctr / average_cpc |

`google_ads_auction_insights_get` / `analyze` は BYOD では空を返します
（Google Ads Scripts からこのデータが取れないため）。競合シェアを見たい
場合は `mureo auth setup` で Live API に切り替えてください。
