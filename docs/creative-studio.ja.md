...
ビジュアル品質の上限はプロンプトの質で決まります。そこで `creative-generate`
スキルは、生成のたびにエージェントが埋める**プロンプト設計フレームワーク**を
同梱しています——スキャフォールド（`[被写体と動き] + [環境] + [スタイル] +
[ライティング] + [配色とムード] + [構図とネガティブスペース] + [品質記述]`）、
スタイル別の語彙メニュー（商品写真・イラスト・3D レンダー・コラージュ/エディトリアル）、
ジャンル別プリセット（美容・B2B SaaS・不動産・EC/食品・採用）、プロバイダごとの
方言メモ（FLUX / gpt-image / Gemini image）、アンチパターン一覧です。作例は
スキルの *Visual prompt engineering* セクションを参照してください。

ネガティブスペースをプロンプトの文言頼みではなく機械的に担保するため、
`creative_studio_generate_visual` は任意の **`template`** 引数
（`hero_overlay` / `split` / `minimal_badge`）を受け取ります。指定すると、その
テンプレートのネガティブスペース指示文がプロンプトへ自動付与され、生成される
被写体は対応レイアウトがコピーを重ねる「余白ゾーン」を空けます。合成に使う
テンプレートを渡したうえで、プロンプト本文にも構図の意図を書き添えてください
——両者が互いを補強します。
