...
| `mureo_state_get` | Read STATE.json as a parsed v2 document (version, platforms, campaigns, action_log, decisions) plus `server_now`, the `path` it read and the runtime `workspace_id` (and `notices` when the runtime has any). Optional `action_log` (`all` / `pending` / `none`) and `decisions` (`all` / `none`) scope the two trails independently to cut context cost | *(none)* |
