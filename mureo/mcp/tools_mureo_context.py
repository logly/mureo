...
TOOLS: list[Tool] = [
    Tool(
        ...
        description=(
            ...
            "A notice means the runtime does not consider this session to "
            "be on the workspace it should be on; workspace-bound skills "
            "stop on it rather than proceed. "
    ...
    Tool(
        ...
        description=(
            ...
            "history and must never be read as 'today'. The response also "
            "names the file it read — ``path`` (the STATE.json consulted) "
            "and the runtime ``workspace_id`` (and ``notices`` when the "
            "runtime has any). ``server_now``, ``path``, ``workspace_id`` "
            "and ``notices`` are response fields only — do not write them "
            "back into STATE.json. A notice means the runtime does not "
            "consider this session to be on the workspace it should be "
            "on; workspace-bound skills stop on it rather than proceed. "
    ...
    Tool(
        ...
        description=(
            ...
            f"{DAILY_RETENTION_DAYS} days in the document and archives older "
            "ones under ``history/daily/<YYYY-MM>.json`` on write. "
