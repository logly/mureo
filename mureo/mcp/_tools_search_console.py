...
TOOLS: list[Tool] = [
    ...
    Tool(
        ...
        description=(
            ...
            "refreshes the sitemap entry so Google will re-crawl it; "
            "idempotent (resubmitting the same feedpath updates the last-"
            "submitted timestamp rather than duplicating). Returns an empty "
            "object on success; the API gives no synchronous processing "
            "status. Does not fetch or validate the sitemap contents — that "
            "happens asynchronously on Google's side and results surface in "
