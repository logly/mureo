...
class SecretStore(Protocol):
    ...
    - ``multi_account_auth: bool`` — set ``True`` to mark the store as a
      ...
      OAuth client (plus the optional legacy ``developer_token``), one
      Meta app, serving N clients whose ``customer_id`` /
      ``account_id`` are supplied per-request out of band). The
      ``mureo configure`` OAuth flow then persists only the shared
      credentials and skips the per-account picker, redirecting
      straight to ``/done`` (#198). Honored only when exactly ``True``
      (see
