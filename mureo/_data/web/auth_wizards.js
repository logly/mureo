...
// is active.
//
// The Meta-specific pieces (hosted-connector setup card, connection-method
// chooser, system-user token card) live in auth_wizards_meta.js and are
// reached through window.MUREO_AUTH_META; that file must load first.
...
(function () {
  ...
  function buildProviderInstallSlot(state, platform, onComplete) {
    ...
    if (isHosted) {
      ...
      if (!window.MUREO_AUTH_META.showManualSetup(wrap, state, providerId)) {
        onComplete();
      }
      return wrap;
    }
  ...
  function renderStepWizard(slot, state, onAllDone, outerState) {
    ...
    if (slot.oauthProvider) {
      ...
      if (slot.oauthProvider === "meta") {
        ...
        const tokenCard = window.MUREO_AUTH_META.buildMetaTokenCard(onAllDone);
        ...
        wrap.appendChild(
          window.MUREO_AUTH_META.buildMetaMethodChooser(
            tokenPanel, oauthPanel, tokenCard
          )
        );
