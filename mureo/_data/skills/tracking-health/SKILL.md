...
13. **Persist the report summary** (best-effort): Call `mureo_state_report_set` with `report="tracking"` and a concise `summary` object so the read-only dashboard can render this audit without re-running you. Follow this convention:
    ...
    - `kpis`: per-platform tracking health counts (e.g. `{"meta_ads": {"pixels_ok": 1, "broken": 1}}`), the tracking-parameter consistency counts from step 8 (`{"google_ads": {"tracking_findings": 3, "ads_unchecked": 0}}`) and any ads-vs-GA4 divergence
