...
7. **Recommendations — rotate / refresh, with the evidence as the brief**: For each FATIGUED (and borderline WATCH) ad, hand off to **`/creative-refresh`** — passing the fatigue evidence (frequency, CTR decline, the tiring asset) as the input brief for copy / headline refresh and RSA asset swaps. Do not silently produce new creative here — this skill diagnoses and routes; the creative skill generates under its own approval gate.
