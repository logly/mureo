...
4. **Identify problem campaigns across all platforms**: For each configured ad platform:
   - **Analytics-module check (external integrations only)**: For every external-integration platform (plugin or official MCP), call `mureo_analytics_modules_list` first. Run the rescue diagnostic only when the platform's module advertises `detect_anomalies`; otherwise emit `analytics_not_available_for_<platform>` in the rescue summary rather than fabricating heuristics from the integration's tool schemas (Issue #120).
