...
- **Include** the basic listing / performance / reporting the connector's tools support (campaigns, spend, conversions). Drive it with the connector's own tools as their names/descriptions imply — **best-effort**; never fail the whole workflow because a hosted tool is missing, report it and continue with the other platforms. Reporting it means **naming the platform and saying it was not checked** — a connector disconnected host-side leaves no tool to call and so no error to catch, and the platform is still enumerated from its STATE.json key alone. Never fill that hole: no estimate, no extrapolation, no figure carried over from another period or another platform, and none of the attainment percentages, pass/fail marks or recommendations that only a measured pull earns.
