...
0. **Establish today**: call `mureo_state_get` **first, on every host** (including Claude Code, where you would otherwise `Read` the file) and take `server_now` from its response — ISO 8601 with UTC offset, e.g. `2026-07-28T10:12:33+09:00`. Its date is the **only source of the current date** for this run: the scan window in step 1, the detection *lag* in step 2, and every "has this action's `observation_due` window closed?" judgement are measured from it. Do not shell out (this skill must run in Bash-less headless hosts). The `action_log` timestamps and `last_synced_at` you reconstruct the timeline from are **history** — that is exactly what makes them useful here, and exactly why they are never "now". **Never write `server_now` into STATE.json**: it is a response field, and a persisted copy becomes tomorrow's stale "today".
