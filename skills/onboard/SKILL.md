...
5. **Initialize STATE.json**: For each discovered platform, snapshot campaigns into STATE.json under the corresponding `platforms` key (e.g. `google_ads`, `meta_ads`, `tiktok_ads` for the TikTok hosted connector, `plugin:mureo-amazon-ads-bridge` for Amazon Ads and `plugin:<dist>` for any other plugin platform).
...
8. **Initial diagnosis**: Run health checks on each configured ad platform:
   ...
   - **Amazon Ads (official-MCP bridge, `plugin:mureo-amazon-ads-bridge`)**: drive it best-effort with Amazon's own bridged tools for a comparable 30-day window and record baseline per-campaign spend/conversions. mureo-only anomaly detection / RSA audit do not apply — report `analytics_not_available_for_plugin:mureo-amazon-ads-bridge` and keep the diagnosis to the raw numbers (the generic `analysis_anomalies_check` still works once history exists). See `../_mureo-shared/SKILL.md` → *Plugin platforms*.
