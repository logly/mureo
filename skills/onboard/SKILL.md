...
7. **Offer budget guardrails** (optional but recommended): mureo can enforce **hard** budget limits deterministically on its native platforms — the built-in `StrategyPolicyGate` refuses any native `google_ads_*` / `meta_ads_*` budget mutation that violates a `## Guardrails` section in STRATEGY.md, *before* dispatch, regardless of what the AI decides. The gate also reaches plugin / bridged platforms such as Amazon Ads, but with a weaker, best-effort check — see the honest-scope note below before promising a cap there. Without the section the gate is fail-open (no enforcement), so most operators never get guardrails unless they know to ask — offer them here.
   ...
   - **Honest scope**: the gate reaches every call mureo dispatches, but the **strength of the check differs by platform** — exact argument keys on native `google_ads` / `meta_ads` (**hard enforcement**), best-effort pattern-matched keys on plugin / bridged platforms (**strong but not guaranteed**), and hosted connectors bypass mureo entirely (**no gate at all**). Concretely: for native `google_ads` / `meta_ads` the gate matches the **exact argument keys** it knows, so coverage is deterministic. For mureo-dispatched **plugin / bridged** platforms (including Amazon Ads, `plugin:mureo-amazon-ads-bridge`) it matches budget/bid-like argument keys **best-effort** by pattern — a tool that declares its budget / bid keys in its MCP metadata is matched exactly and that declaration wins, otherwise coverage depends on how the tool names its arguments; tell the operator those caps are **strong but not guaranteed**, and to verify the values after the first mutations on such a platform. Hosted / official MCPs registered with the host (TikTok `tt-ads-*`, `google-ads-official`, `meta-ads-official`) go client→platform and never reach the gate, so for those guardrails stay best-effort until the MCP gateway (#359) lands — say so.
