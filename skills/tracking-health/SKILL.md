...
0. **Establish today**: call `mureo_state_get` **first, on every host** (including Claude Code, where you would otherwise `Read` the file) and take `server_now` from its response — ISO 8601 with UTC offset, e.g. `2026-07-28T10:12:33+09:00`. Its date is the **only source of the current date** for this run: judging whether a pixel's `last_fired_time` is "recent" is a comparison against it, and the `observation_due` you write in step 12 is `server_now`'s date + 7 days. Do not shell out (this skill must run in Bash-less headless hosts) and do not read the date off STATE.json — `last_synced_at`, `reports.*.period` and `action_log` timestamps are **history**, never evidence of what day it is now. **Never write `server_now` into STATE.json**: it is a response field, and a persisted copy becomes tomorrow's stale "today".

1. **Load context**: Read STRATEGY.md (especially each Goal's declared conversion definition, Persona, and Operation Mode) and STATE.json (the same `mureo_state_get` response from step 0 on MCP hosts). Note **what the operator considers a real conversion** for each platform (a lead form submit, a purchase, a phone call) — the audit below is only meaningful relative to that intent.
...
7. **Plugin & hosted-connector platforms** (best-effort): for a plugin platform (`plugin:<dist>:<provider>`) or a hosted connector (`tiktok_ads`), report only the basic tracking status its own tools expose and emit `analytics_not_available_for_<platform>` for the mureo-only deep checks (pixel health, `result_indicator`, conversion-action recency) — do **not** fabricate a tracking verdict from an integration's tool schemas. See `../_mureo-shared/SKILL.md` → *analytics-module parity* and *Hosted-connector platforms*. The one exception is step 8's tracking-parameter consistency check: it is platform-neutral and analyses ad records **you** pass in rather than a per-platform analytics module, so it applies to a plugin or hosted platform exactly as it does to Google and Meta — whenever that platform's own tools can list ads with their destination URLs.

8. **Tracking-parameter consistency — every platform** (`analysis_tracking_consistency_check`): steps 3-7 ask whether the *conversion signal* arrives. This step asks whether the click that produced it lands in the right **row** of the analytics everyone downstream reads. An ad uploaded into campaign B carrying campaign A's `utm_campaign` breaks segment-level reporting while delivery, spend and conversions all look perfectly healthy — nothing appears broken, so nobody investigates.

   - **Collect ad records** for every platform you enumerated in step 2, into one platform-neutral list. Each record is `{ad_id, campaign_id, campaign_name, platform, final_urls, impressions}`:
     - **Google Ads**: `google_ads_ads_list` returns `final_urls`, `campaign_id` and `campaign_name` directly.
     - **Meta Ads**: `meta_ads_ads_list` — the link is in `creative.object_story_spec` (`link_data.link`, or the call-to-action link on `video_data` / `photo_data` / `template_data`), and `creative.url_tags` is appended to it at delivery time, so pass the link **with** `url_tags` merged onto its query string.
     - **Plugin platforms**: use the plugin's own ad-listing tool and its `final_url` field; pass the canonical `plugin:<dist>:<provider>` key as `platform` (see `../_mureo-shared/SKILL.md`).
     - **Bridged / hosted connectors**: pass whatever destination URL the connector reports. **The Amazon Ads bridge exposes no destination-URL field** — pass those ads with `final_urls: []` so they are reported as *unchecked* rather than clean.
     - **`impressions` is what grades the finding**: supply it per ad (a per-ad performance report over the audited window) so mureo can tell a served mis-tagged ad — a data-integrity incident needing a reporting caveat — from one that never served, a cheap fix. Omit it only when the platform cannot give it; omitted is **not** the same as `0`.
   - **Declared convention**: if STRATEGY.md has a `## Tracking Convention` section, pass that text as `convention_markdown` **verbatim**. Do not interpret it yourself — mureo parses it. Without one, the zero-configuration checks still run.
   - **Report what comes back, and only that.** Findings carry `severity`, `delivery_state`, the ad ids and the evidence (which parameters differ, which campaign the borrowed scheme belongs to, which landing page two schemes collided on). mureo deliberately does **not** say which of two schemes is correct — put both in front of the operator. Never add a finding of your own by eyeballing utm values: guessing an account's convention is the failure this check replaces. In particular, only `utm_source` / `utm_medium` / `utm_campaign` identify a campaign; `utm_content` and `utm_term` vary per creative and per keyword by design and are deliberately not compared, so do not report them as inconsistencies.
   - **`ads_without_readable_url` is not a pass.** List those ads explicitly as *not checked* on the platform they came from, and carry the tool's `notes` into the scorecard evidence.
   - Full list of what this check can and cannot detect: `docs/tracking-consistency.md`.

9. **Scorecard**: present a per-platform **tracking scorecard**, one row per check, each rated **OK** / **Watch** / **Broken**:
   ```
   ...
   Meta Ads    URL tracking consistency           OK       0 findings / 34 ads
   ...
   Google Ads  URL tracking consistency           Broken   16 ads carry campaign A's utm_campaign
   Amazon Ads  URL tracking consistency           n/a      not checked — bridge exposes no URL
   ```
   Rate the consistency row **Broken** when any finding has `delivery_state: served` (reporting is already wrong), **Watch** when findings exist but nothing served, and **OK** only when the platform's ads were all readable and produced no findings.

10. **Fix list — ranked by revenue risk**: turn every **Broken** / **Watch** into a concrete fix, ordered by how much spend is riding on the broken signal (a broken primary action on the highest-spend campaign is #1; a missing secondary action on a paused campaign is last). For each fix state: what is wrong, the concrete remediation, and the platform/UI or tool that performs it.

11. **Approval gate for any state-mutating fix**: most remediations here are **operator actions** (re-add a tag on the site, flip an action to primary in the Google Ads UI, install CAPI). Where a fix *does* mutate platform state through a mureo tool (e.g. `google_ads_conversions_update` to re-enable an action, `google_ads_conversions_create` to add one, sending backfill events via `meta_ads_conversions_send`), apply the *Confirm Before Write Operations* rule from `../_mureo-shared/SKILL.md` — show the current value and the proposed change and get explicit **approval** before the call. For an official/hosted-MCP write, also self-apply the STRATEGY.md `## Guardrails` rules yourself (mureo's PolicyGate cannot see that call) — see `../_mureo-shared/SKILL.md` → *Hosted-connector platforms*. A plugin / bridged write (including Amazon Ads) does reach the gate — **exactly** on the money tools mureo declares paths for (the 13 money-carrying Amazon tools), with the pattern scan still applied underneath as a floor, and best-effort on argument-key patterns alone for anything else — so still state the applicable cap.

12. **Record any change to `action_log`**: for each approved state-mutating fix, log to `action_log` (via `mureo_state_action_log_append`) with `metrics_at_action` (the pre-fix conversion counts / firing state you could read) and an `observation_due` **7 days** out from `server_now`'s date, so daily-check's evidence step confirms the fix actually restored the signal. A pure audit with no changes still logs a summary entry (findings + scorecard verdict), with no `observation_due`.

13. **Persist the report summary** (best-effort): Call `mureo_state_report_set` with `report="tracking"` and a concise `summary` object so the read-only dashboard can render this audit without re-running you. Follow this convention:
    ...
    - `kpis`: the per-platform breakdown, not the headline row — tracking health counts (e.g. `{"meta_ads": {"pixels_ok": 1, "broken": 1}}`), the tracking-parameter consistency counts from step 8 (`{"google_ads": {"tracking_findings": 3, "ads_unchecked": 0}}`) and any ads-vs-GA4 divergence
    - `flags`: notable items (e.g. `["meta_lead_event_missing", "google_primary_action_zero_conv", "tracking_parameter_mismatch"]`)
    - `narrative`: the overall verdict (OK / Watch / Broken) and what you propose next, **at most 400 characters** — the tool refuses a longer one rather than truncating it (a sentence cut in half is worse than a long one). Do not restate the figures and do not list the findings here: numbers belong in `totals`, findings in `flags`.
