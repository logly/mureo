...
8. **Show diff**: Compare old vs new state and highlight changes:
   ...
   - **Ad-level status changes** — ads added or removed, paused or resumed, and ads that stopped delivering (`effective_status` no longer `ACTIVE`), diffed against the `ads` the previous run stored. Changes mureo did not make are the point of this line: name them explicitly (see *Mixed operation*) instead of letting the new snapshot quietly replace the old.
