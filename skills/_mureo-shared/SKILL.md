...
- **Guardrails DO reach these platforms — but best-effort, not the hard check.** Because a plugin / bridged tool call goes through mureo's own dispatcher, `StrategyPolicyGate` applies STRATEGY.md `## Guardrails` to it — matching budget/bid-like argument keys **by pattern** (a tool that declares its budget / bid keys in its MCP metadata is matched exactly and that declaration wins). That is weaker than the **exact-argument-key** enforcement the gate does on native `google_ads_*` / `meta_ads_*`: exact coverage here depends on how the tool names its arguments, so treat a plugin-platform cap as **strong but not guaranteed** — say so in the output and verify the resulting values after the first mutations. Weaker still is a **hosted connector**, which mureo cannot gate at all — see below.
