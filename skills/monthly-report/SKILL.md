...
4. **Month-over-month (MoM) comparison** — the reporting month vs the month **before** it, using only windows the tools genuinely support (same honesty rule as the period above):
   ...
   - **Google Ads**: `google_ads_performance_report`'s `period` accepts an explicit range in GAQL spelling — `period="BETWEEN 'YYYY-MM-DD' AND 'YYYY-MM-DD'"`, both endpoints inclusive — alongside the date-range constants (#716). So pull the month-before with its exact first/last dates, computed by stepping back two months from `server_now`'s month and never from a date found in STATE.json, and compare against `LAST_MONTH`. A true Google Ads MoM is available. If the explicit-range call fails, fall back in order to (a) STATE.json's persisted prior-month rollup (`platforms[<p>].periods[...]`) or the previous monthly report's `kpis` (from `mureo_state_report_set` history); (b) **state that the Google Ads MoM comparison is unavailable this month** rather than mislabel `LAST_90_DAYS` (a 3-month blend) as "the prior month". Never silently substitute a different window.
