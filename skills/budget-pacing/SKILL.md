...
10. **Execute approved changes** with each platform's budget tools:
    ...
    - **Hosted / official writes**: self-apply the `## Guardrails` rules yourself first (mureo's PolicyGate does not see off-path calls) and confirm — see `../_mureo-shared/SKILL.md` → *Hosted-connector platforms*. **Plugin / bridged writes** (including Amazon Ads) *do* go through mureo's dispatcher, so the gate applies — but best-effort on argument-key patterns, so state the cap and verify it after the change.
