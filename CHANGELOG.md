...
- **BYOD Meta `get_performance_report` now surfaces `result_indicator`** — the per-campaign output dict gained a `result_indicator` field (e.g. `actions:link_click`, `actions:offsite_conversion.fb_pixel_lead`, or empty when the campaign carries no conversion-event optimization). The value was already written to `metrics_daily.csv` by the Phase 3 importer but was being dropped before reaching the agent, so `/daily-check` could see "42 results vs 3 results" across two campaigns with no signal that the units were incomparable. With the indicator surfaced, the agent can now detect CV-definition mismatches (e.g. a `link_click`-optimized campaign masquerading as a high-CV-rate winner against a true lead-optimized sibling), which is critical for budget-rebalance and goal-evaluation decisions. The first non-empty indicator seen for each campaign (across the period's daily rows) is exposed; if every row is empty, the field is the empty string. Regression test in `tests/test_byod.py::test_byod_meta_get_performance_report_exposes_result_indicator`.

### Changed
- **All bundled command skills now name the specific MCP tools to call** — `mureo/_data/commands/{daily-check,search-term-cleanup,budget-rebalance,competitive-scan,creative-refresh,rescue,sync-state,weekly-report,onboard}.md` previously instructed the agent to "use the platform's analysis tools" / "use the platform's health check tools" without naming them. Real BYOD sessions hit a reproducible failure mode where the agent (a) looked for raw CSV files in the project directory, (b) failed to find any (BYOD data lives under `~/.mureo/byod/`, not in the project), and (c) declared the command unrunnable — even though the corresponding MCP tool would have returned full data. The skill body now lists the concrete tool names (`google_ads.search_terms.report`, `meta_ads.insights.report`, `google_ads.zero_conversions.diagnose`, etc.) per platform, plus an explicit reminder that mureo BYOD data is centralized under `~/.mureo/byod/` and is only accessible through MCP tools. Each command also documents which BYOD tools return `[]` by design (auction insights, RSA asset ratings, etc.) so the agent doesn't interpret an empty list as "data unavailable" and abort. No code changes — Markdown skill bodies only.

## [0.7.0] - 2026-04-29

### Added
- **BYOD Meta Ads adapter** — `mureo/byod/adapters/meta_ads.py` consumes the user's Ads Manager Excel export (Reports → Customize → Export → Excel) and normalizes it to CSVs under `~/.mureo/byod/meta_ads/`. Identity (campaign_id / ad_set_id / ad_id) is synthesized from name via deterministic SHA-256 hash so re-imports keep stable IDs. **Multilingual header support** — recognizes column names in English / 日本語 / Español / Português / 한국어 / 繁體中文 / 简体中文 / Français / Deutsch (e.g. キャンペーン名, インプレッション, 消化金額 (JPY), 結果), verified against actual exports in each locale. Multiple rows per (day, campaign) — typical when Ad-set or Ad breakdown is enabled — are summed before write. Pivot subtotal rows (date cell = `All` or locale equivalent) are skipped automatically. Currency is JPY-only; non-JPY symbol prefix (`$`, `€`, `£`, …) raises `UnsupportedFormatError` to prevent silent over/under-reporting. Disjoint from the Google Ads adapter via the long-form vs short-form campaign header distinction, so a single workbook can carry both adapters' data.
