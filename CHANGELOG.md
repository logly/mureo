...
### Added
- **BYOD Meta Ads adapter** — `mureo/byod/adapters/meta_ads.py` consumes the user's Ads Manager Excel export (Reports → Customize → Export → Excel) and normalizes it to CSVs under `~/.mureo/byod/meta_ads/`. Identity (campaign_id / ad_set_id / ad_id) is synthesized from name via deterministic SHA-256 hash so re-imports keep stable IDs. **Multilingual header support** — recognizes column names in English / 日本語 / Español / Português / 한국어 / 繁體中文 / 简体中文 / Français / Deutsch (e.g. キャンペーン名, インプレッション, 消化金額, 結果), verified against actual exports in each locale. Multiple rows per (day, campaign) — typical when Ad-set or Ad breakdown is enabled — are summed before write. Pivot subtotal rows (date cell = `All` or locale equivalent) are skipped automatically. **Currency-agnostic spend column** — Meta exports the spend header as `Amount spent (XXX)` where `XXX` is the account's ISO currency code (`JPY` / `USD` / `EUR` / `GBP` / `KRW` / `INR` / etc.); `_resolve_spend_idx` strips the suffix before alias matching, and `_to_float` strips a leading currency symbol (¥ / $ / € / £ / ₩ / ₹ / ¢ / etc.) from cell values. Cost values are stored raw in the account's own currency — cross-account currency conversion is out of scope. Disjoint from the Google Ads adapter via the long-form vs short-form campaign header distinction, so a single workbook can carry both adapters' data.
...
- **Resolved 8 CodeQL Code Scanning alerts** in OAuth + GAQL paths:
  ...
  - `py/clear-text-logging-sensitive-data` (4 errors) — OAuth URL-rejection logs and GAQL `_search` start/done logs no longer echo any value derived from the URL or query string. The terminal `print` of the Meta OAuth URL in `run_meta_oauth` was removed entirely (the browser is the only consumer of the URL).
