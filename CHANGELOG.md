...
### Added
- **BYOD Meta Ads adapter** — `mureo/byod/adapters/meta_ads.py` consumes the user's Ads Manager Excel export (Reports → Customize → Export → Excel) and normalizes it to the 4 CSVs the existing `ByodMetaAdsClient` already reads under `~/.mureo/byod/meta_ads/`: `campaigns.csv`, `ad_sets.csv`, `ads.csv`, `metrics_daily.csv`. Identity (campaign_id / ad_set_id / ad_id) is synthesized from name via deterministic SHA-256 hash so re-imports keep stable IDs. Recognized columns (English only in v1): Day / Reporting starts / Date, Campaign name, Ad set name (optional), Ad name (optional), Impressions, Clicks (all) / Link clicks / Clicks, Amount spent (JPY) / Spend, Results / Conversions. Multiple rows per (day, campaign) — typical when Ad-set or Ad breakdown is enabled — are summed before write. Disjoint from the Google Ads adapter via the `Campaign name` (long form) vs `campaign` (short form, used by the Google Ads Script) header distinction, so a single workbook can carry only one adapter's data.
