...
### Added
- **BYOD (Bring Your Own Data) mode** — drop a Google Ads CSV export into mureo and analyse it locally without OAuth, a developer token, or any SaaS sign-up. New CLI: `mureo byod import <file.csv>` (auto-detects format) with `--google-ads` / `--meta-ads` / `--search-console` overrides, plus `mureo byod status / remove --<platform> / clear`. Activation is by data presence (`~/.mureo/byod/manifest.json`) — the MCP server detects it per-platform at every dispatch via the new `mureo/mcp/_client_factory.py`. Real-API credentials and BYOD imports can coexist: e.g. Google Ads = BYOD CSV, Meta Ads = real API, Search Console = real API in a single `/daily-check` call. Phase 1a ships the Google Ads Report Editor adapter; Meta Ads + Search Console adapters and `docs/byod.md` walkthrough land in Phase 1b/2. PII column rejection (`email`/`phone`/`user_id`/`ip_address`/`customer_email`), report-preamble skipping, total/合計 row skipping, mutation-tool blocking (`{"status": "skipped_in_byod_readonly"}` so a curious agent never accidentally writes), stale-manifest detection, path-traversal defense in the installer, and SHA256 source-file dedupe all included. Real-mode users see no behavior change.
