...
さらにmureoには学習の仕組みがあります。エージェントの分析を修正したり運用上の気づきを共有すると、`/learn-diagnosis` でナレッジベースに保存できます。保存した知識は次回以降のセッションで自動的に読み込まれるため、使い続けるほどエージェントがあなたのアカウントの特性を理解し、より的確な判断ができるようになります。
...
エージェントの分析を修正したり、運用で気づいたことを `/learn-diagnosis` でナレッジベースに保存できます。保存した知識は次回以降のセッションで自動的に読み込まれるため、同じ間違いを繰り返しません。1つのキャンペーンで得た知見が、アカウント内の似た状況にも活かされます。
...
| `/learn-diagnosis` | 運用で得た知見をナレッジベースに保存。次回以降のセッションに自動で反映 |
...
| `mureo-pro-diagnosis` | 運用で蓄積するナレッジベース（`/learn-diagnosis` で記録） |
