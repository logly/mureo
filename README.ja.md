...
|---|---|---|
| **戦略ガード** | `STRATEGY.md` と `policy.yaml` を読み、ペルソナ・USP・ブランドボイス・予算ルール・運用モードに照らして全変更をゲートする | 公式 MCP はあなたのビジネスを知らない |
| **成果連携** | プラットフォーム指標と CRM / 売上 / LTV をローカルで相関付ける | 公式 MCP はプラットフォーム側の数字しか見ない |
| **監査と証跡** | 「いつ・誰が・なぜ・どんな根拠で」を追記専用の署名付きログに残す（diff / replay / rollback 可能） | 公式 MCP は戦略的意図を残さない |
