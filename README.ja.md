...
広告アカウントは攻撃者から見て価値の高い標的です。mureo はAIエージェントによる運用を前提に、多層防御を組み込んで設計されています。

- **資格情報ガード** — `mureo setup claude-code` で `~/.claude/settings.json` に PreToolUse フックを追加し、`~/.mureo/credentials.json` や `.env` など機密ファイルの読み込みをAIエージェントからブロックします。プロンプト・インジェクションによるトークン漏洩を、ファイルシステム経由の経路で防ぎます。
- **GAQL入力検証** — Google Ads クエリ（GAQL）に入るID・日付・期間定数・文字列リテラルは、すべて単一のホワイトリストベース検証層（`mureo/google_ads/_gaql_validator.py`）を通ります。`BETWEEN` 句も生の入力を通さず、両側の日付をパターンマッチ＋再検証します。
- **異常検知** — `mureo/analysis/anomaly_detector.py` が action_log 履歴から中央値ベースのベースラインを作り、現在のキャンペーン指標と比較して「ゼロ支出」「CPA急騰」「CTR急落」を優先度付きで通知します。サンプルサイズのゲートにより単日ノイズを抑制します。
- **不変データモデル** — 状態オブジェクト（`StateDocument`, `ActionLogEntry`, `CampaignSnapshot`, `Anomaly` など）は `frozen=True` のdataclassで、エージェントが記録を密かに書き換えることを防ぎます。
- **ローカル完結の資格情報** — トークンは `~/.mureo/credentials.json` または環境変数から読み込み、広告プラットフォームの公式APIにのみ送信します。mureo 自身はテレメトリも外部通信も持ちません。

脅威モデルの詳細と脆弱性報告手順は [SECURITY.md](SECURITY.md) を参照してください。
